...
export function validateSafeCommand(command: string, args: string[] = []) {
  ...
  if (blocked) throw new Error(`Command rejected by safety policy: ${blocked}`);
...
export function parseSafeCommand(input: string) {
