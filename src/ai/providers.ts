...
export const AI_MODEL_DEFAULTS: Record<AiProvider, string[]> = {
...
export function getProviderConfig(provider?: string, model?: string): OpenArvaProviderConfig {
...
export function saveOpenArvaConfig(config: OpenArvaProviderConfig) {
  const configPath = getOpenArvaConfigPath();
...
export function createModelProvider(config: OpenArvaProviderConfig): LanguageModel {
  ...
  switch (provider) {
    case 'anthropic':
      return createAnthropic({ apiKey: config.apiKey || getEnvValue('ANTHROPIC_API_KEY') })(
        modelName,
      );
    case 'gemini':
      return createGoogleGenerativeAI({ apiKey: config.apiKey || getEnvValue('GEMINI_API_KEY') })(
        modelName,
      );
    case 'groq':
      return createOpenAI({
